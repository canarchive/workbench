    ...
    'columns' => [
		...
		[
			...
			'value' => function($model) {
				return "<a href='{$model->infoUrl}' target='_blank'>{$model->name}</a>";
			},
		],
		[
			'format' => 'raw',
        ...
        [   
            ...
            'value' => function($model) {
                ...
                return '<input name="orderlist" type="text" style="width:80px;" value="' . $model->orderlist . '" class="input-text-c" onchange="updateElemByAjax(\'' . $updateUrl . '\', ' . $model->id . ', \'orderlist\', this.value);">';
            },  
        ],
		[
        ...
        [   
            ...
            'value' => function($model) {
				...
				$opeStr = "<a href='{$listMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>{$model->num_comment}</a>";
                return $opeStr;
            },  
        ],
        [   
            ...
            'value' => function($model) {
				...
				$opeStr = "<a href='{$listMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>{$model->num_realcase}</a>";
                return $opeStr;
            },  
        ],
        [   
            ...
            'value' => function($model) {
				...
				$opeStr = "<a href='{$listMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>{$model->num_working}</a>";
                return $opeStr;
            },  
        ],
        [   
            ...
            'value' => function($model) {
                ...
                $opeStr = "<a href='{$addMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>添加</a>";
				$opeStr .= "---<a href='{$listMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>{$model->num_designer}</a>";
                return $opeStr;
            },  
        ],
        [   
            ...
            'value' => function($model) {
                ...
                $opeStr = "<a href='{$addMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>添加</a>";
				$opeStr .= "---<a href='{$listMenu['url']}?city_code={$cityCode}&merchant_id={$model->id}'>{$model->num_owner}</a>";
                return $opeStr;
            },  
        ],
    ],
