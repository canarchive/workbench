...
<tbody>
    ...
    <tr id='node-<?= $info['id']; ?>' class="<?= $info['parentNode']; ?>">
        ...
        <td>
        <?php
        $sourceUrl = $model->getSourceUrl($info);
        $nowUrl = $model->getNowUrl($info);
        $redirectUrl = $model->getRedirectUrl($info);
        echo "<a href='{$nowUrl}' target='_blank'>新Url</a>--";
        echo "<a href='{$redirectUrl}' target='_blank'>模拟跳转Url</a>--";
        echo "<a href='{$sourceUrl}' target='_blank'>源Url</a>";
        ?>
        </td>
        <td>
        <?php
