...
class Spider
{
    ...
    public function downFiles()
    {
        ...
        foreach ($datas as $key => $file) {
            ...
            $baseUrl = strpos($url, '?') !== false ? substr($url, 0, strpos($url, '?')) : $url;
            if (strpos($url, 'tugao.com')) {
                continue;
            }
            $basefile = basename($file);
            $basefile = strpos($basefile, '?') !== false ? substr($basefile, 0, strpos($basefile, '?')) : $basefile;
            if (isset($extDatas[$basefile]) && $baseUrl != $extDatas[$basefile]) {
                echo "<a href='{$url}' target='_blank'>{$url}</a>--<a href='{$extDatas[$basefile]}' target='_blank'>{$extDatas[$basefile]}</a><br />";
                $basefile = 'k' . $key . '_' . $basefile;
            }
            $extDatas[$basefile] = $baseUrl;
